...
function f1() {
	let out = '';
	for (let i = 0; i < a1.length; i++) out += a1[i] + ' ';
...
function f2() {
	let out = '';
	for (let i = 0; i < a2.length; i++) out += `${i} ${a2[i]} `;
...
function f4() {
	...
	for(let i = 0; i < divS.length; i++) divS[i].textContent += out;
}
...
function f5() {
	let newArr = [];
	for (let item of a5) if (item > 7) newArr.push(item);
	return newArr;
}
...
function f6() {
	...
	for (let i = 0; i < newArr.length; i++) out += newArr[i] + ' ';
...
function f7() {
	let newObj = {};
	for (let item of a7) newObj[item.id] = item.name;
	return newObj;
}
...
function f8() {
	let newArr = [];
	for (let item of a8) newArr.push(item.id);
...
function f9() {
	let out = 0;
	for(let item of a9) {
		if (item.length > out) out = item.length - 1;
	}
	console.log(a9)
	return out;
}
...
function f15() {
	...
	for(let item of elements) item.textContent = 15;
