...
const f4 = () => {
	const result = s4.has(document.querySelector('.i-4').value);
...
const f7 = () => {
	let out = document.querySelector('.out-7'),
		 password = document.querySelector('.i-7').value;
		 check = new Set()

	for (let i = 0; i < password.length; i++) {
		check.add(password[i]);
	}

	if (check.size === password.length && password.length > 6) {
		out.innerHTML = 1;
	} else {
		out.innerHTML = 0;
	}
